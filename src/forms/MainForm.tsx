...
const MainForm = (props: Props) => {
  ...
  return (
    ...
    <form ref={form} onSubmit={handleSubmission} className={`font-ultra gap-4 grid grid-cols-2 rounded md:rounded-xl text-zinc-800 rounded-bl-xl  ${props.moreClasses}`}>
      <label htmlFor="" className="col-span-1">
        <Input type={'text'} value={formName ? formName : ''} onChange={handleNameChange} placeholder={"Name please  "} additionalClasses={'rounded-r-lg rounded-bl-lg rounded-tl-4xl bg-white'} name={'name'} />
      </label>

      <label htmlFor="" className="col-span-1 ">
        <Input type={'email'} value={formEmail ? formEmail : ''} onChange={handleEmailChange} placeholder={"Enter email here "} additionalClasses={'rounded-l-lg rounded-br-lg rounded-tr-4xl  bg-white'} name={'email'} />
      </label>

      <label htmlFor="" className='col-span-full'>
        <TextArea additionalClasses={'min-h-44 bg-white !rounded-t-lg'} value={formMessage ? formMessage : ''} onChange={handleFormChange} placeholder={"Have something you want to say that cant fit in one line, let me know here please! "} name={'message'} />
      </label>
