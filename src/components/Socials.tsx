...
<>
    {socialElms ? socialElms.map((item)=>(
        <a key={item.id} href={item.url} title={item.title} target={item.target} className={`md:cursor-none w-max h-max bg-red-950 p-2 lg:p-3 rounded-full flex items-center justify-center ${item.desktopHidden ? 'md:hidden' : ''}`} aria-label={item.title} >
            {item.icon}
        </a>
    )):''}
</>
